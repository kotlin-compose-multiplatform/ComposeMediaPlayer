...
1) Run `./gradlew :mediaplayer:publishToMavenLocal`
2) Open `~/.m2/repository/io/github/kdroidfilter/composemediaplayer/"}`
...
4) Add the secrets to `local.properties`:
   ...
   ```
5) Run `./gradlew :mediaplayer:publishAllPublicationsToSonatypeRepository`
